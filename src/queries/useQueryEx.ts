...
import { axiosDelete, axiosGet, axiosPost, axiosPut } from "./axiosEx";
...
const useGetQueryEx = <T>({ queryKey, url, params, enabled=false, onError }: UseGetQueryExProps) => {
...
const usePutQueryEx = <T, U>({ url, contentType, onSuccess, onError }: UsePutQueryExProps<U>) => {
  const { mutate, error, status } = useMutation({
    mutationFn: (reqData: T) => axiosPut<T, U>({ url: url, contentType: contentType, reqData: reqData }),
    onSuccess: (data: U) => onSuccess && onSuccess(data),
    retry: false
  })
